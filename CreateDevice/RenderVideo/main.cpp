...
bool LoadTextureFromBMP(int idx);
...
VOID Render()
{
	...
	if (a % 120 == 0)
	{
		LoadTextureFromBMP(0);
	}
	else if (a % 450 == 0)
	{
		LoadTextureFromBMP(1);
	}
...
bool LoadTextureFromBMP(int idx)
{
	...
	if (!initImage)
	{
		for (int i = 0; i <= 1; i++)
		{
			MYBITMAPINFO* bitmapInfo = new MYBITMAPINFO;
			LPCSTR imgName;
			...
			fread(&bitmapInfo->bFile, sizeof(BITMAPFILEHEADER), 1, fp);
			fread(&bitmapInfo->bInfo, sizeof(BITMAPINFOHEADER), 1, fp);
			fseek(fp, bitmapInfo->bFile.bfOffBits, SEEK_SET);
			bitmapInfo->data = (LPBYTE)malloc(bitmapInfo->bFile.bfSize * sizeof(BYTE));
			fread(bitmapInfo->data, bitmapInfo->bFile.bfSize, 1, fp);
			imges[i] = bitmapInfo;
	...
	MYBITMAPINFO* imgInfo = imges[idx];
	D3DLOCKED_RECT	rect;
	DWORD	w, h;
	INT		x, y;
	LPDWORD	data;
	...
	//24bit 픽셀만 취급한다.
	if (imgInfo->bInfo.biBitCount != 24)
		goto error;

	//텍스쳐 사이즈를 2의 승수에 맞춘다.
	w = imgInfo->bInfo.biWidth;
	h = imgInfo->bInfo.biHeight;
	...
	for (y = (int)imgInfo->bInfo.biHeight - 1; y >= 0; y--)
	{
		//Y값의 시작 위치로 이동한다.
		data = (LPDWORD)((LPBYTE)rect.pBits + rect.Pitch * y);

		for (x = 0; x<imgInfo->bInfo.biWidth; x++, data++)
		{
			BYTE	r, g, b;
			b = imgInfo->data[dataIdx];
			dataIdx++;
			g = imgInfo->data[dataIdx];
			dataIdx++;
			r = imgInfo->data[dataIdx];
			dataIdx++;
